...
const char DEFAULT_WALLET_FILE_NAME[] = "keplwallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.kepl.org:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("keplwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("keplwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("keplwallet.desktop");
  ...
  res = autorunSettings.contains("keplWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("keplWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.KEPL.keplwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=KEPL Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("keplWallet", appPath);
    } else {
      autorunSettings.remove("keplWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\kepl", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:kepl");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\kepl\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\kepl\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("keplwallet.cfg"));
